...
int box_decode::parse_file_desc(box_file_desc_t      *file_desc,  char *data,  unsigned int size)
{
	...
	if( MKTAG(data[0], data[1], data[2], data[3]) == box_type_file_desc)
	{	
...
int box_decode::parse_pack( box_pack_t pack,  char *data, int *headersize)
{		
	if( MKTAG(data[0], data[1], data[2], data[3])== box_type_pack)
	{	
		...
		pack = (box_pack_t *)malloc(MAX_BOX_HEAD_SIZE);
