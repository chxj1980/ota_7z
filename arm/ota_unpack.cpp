...
{	
	...
	while(size--)
	{
		if((tmpdata[1] == 0) && (tmpdata[2] == 0) && (tmpdata[3] == 0)) //find the box flag
		{
			for(; (tmp_box_type + i) < box_type_max; i++ )
			{
				if((tmpdata[0] & 0x000000ff) == tmp_box_type + i) 
				{
	...
	do{
		...
		switch(box_type)
		{
			...
			case box_type_compress: 
				...
				if(decode.get_packet_file(tmp_file, file_path) < 0)
				{
					ERROR("get tar file faile !\n");
					break;
				}

				PRINTF("get uncompress file:%s\n", tmp_file);
				sprintf(tmp_tarfile, "%s/%s", file_path, tmp_file);
				PRINTF("tar file:[%s], [%s]\n", tmp_tarfile, tmp_file);
